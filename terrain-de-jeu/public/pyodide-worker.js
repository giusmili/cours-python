...
function hardenWorkerCapabilities() {
  // Student Python can import the `js` bridge. Keep that bridge useful for
  // basic values while removing the capabilities that could talk to the
  // network, forge worker protocol messages, or load additional scripts.
  self.fetch = denyCapability("Network access");
  self.postMessage = denyCapability("Direct worker messaging");
  self.importScripts = denyCapability("Dynamic script loading");
